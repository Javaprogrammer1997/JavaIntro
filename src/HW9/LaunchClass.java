...
public class LaunchClass {
    public static void main(String[] args) {
        ...
        for(Cars car : cars){
            processor.findCarsByMark(car, "Mercedes");
        }
        processor.listOfCarsThatHaveBeenOperatedForMoreYears(cars, "Galardo", 5);
        for(Cars car: cars){
            processor.listOfCarsThatHaveBeenOperatedForMoreYears(car, "Galardo", 5);
        }
        processor.searchForCarsForMoreThanTheSpecified(cars, 2017, 100000);
        for(Cars car : cars){
            processor.searchForCarsForMoreThanTheSpecified(car, 2017, 100000);
        }
    }
}
